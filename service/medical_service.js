			...
			switch (key) {
				case 'bun':
					if (value > 20 || value < 8) {
						...
						responseText +=
							'참고치 정상 범위 에 속하는 수치입니다. 자세한 내용이 궁금하다면 대화창에 검사명을 입력해보세요. (단, 참고치는 검사가 진행되는 검사실에 따라 달라질 수 있으므로 전문의와의 상담이 필요합니다)\n\n';
						baseFormat.title = 'BUN';
						baseFormat.description =
							'참고치 정상 범위 에 속하는 수치입니다. 자세한 내용이 궁금하다면 대화창에 검사명을 입력해보세요. (단, 참고치는 검사가 진행되는 검사실에 따라 달라질 수 있으므로 전문의와의 상담이 필요합니다)';
					}
				...
				case 'phosphate':
					if (value < 2.5 || value > 4.5) {
						responseText +=
							'신장의 인 배설기능이 감소하면 혈액 내 인 수치가 올라갈 수 있어요. 상승한 인 수치 그 자체로 신장에 독성이 있을 수 있고, 뼈 녹이는 작용이 있어 골절 등 합병증이 동반될 수 있어, 정기검사로 전문의와 상담,식이,약물요법 등 관리가 필요해요.\n\n';
				...
				case 'egfr':
					if (value > 90) {
						...
						responseText +=
							'신기능이 다소 감소했습니다. 극심한 피로, 식욕 감소, 가려움증이 발생할 수 있어요. 혈압을 철저히 관리하고, 신장기능 악화를 늦추기 위해 정기적인 검사와 전문의 상담으로 적극적인 치료에 노력을 멈추지 마세요.\n\n';
						baseFormat.title = 'eGFR';
						baseFormat.description =
							'신기능이 다소 감소했습니다. 극심한 피로, 식욕 감소, 가려움증이 발생할 수 있어요. 혈압을 철저히 관리하고, 신장기능 악화를 늦추기 위해 정기적인 검사와 전문의 상담으로 적극적인 치료에 노력을 멈추지 마세요.';
				...
				case 'total_co2':
					if (value < 22 || value > 29) {
						responseText +=
							'중탄산염은 신장에서 배설되고 재흡수 돼요. 농도 높낮음은 몸이 산-염기 균형을 유지 또는 전해질 균형이 깨짐을 의미해요. 높은 혈중 총 이산화탄소 농도는 떨림,호흡곤란 등 증상으로 이어질 수 있어 정기검진을 통한 전문의 상담이 필요해요\n\n';
				...
				case 'bunCrRatio':
					if (value < 20) {
						responseText +=
							'체내 수분 양을 간접적으로 볼 수 있는 지표예요. 상승한 경우 탈수를 의심해볼 수 있지만 중증 신부전이 동반된 경우 수치 해석에 주의를 기울여야 해요. 기준 이상의 상승이 있다면 탈수 교정 후 재검사가 추천되며 전문의 진료가 필요해요.\n\n';
