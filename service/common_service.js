...
const CommonService = {
	...
	getDBLiteral: function (literal) {
		return sequelize.literal(literal);
	},
	startTransaction: async function (req) {
		if (this.isEmpty(req.transaction) == true) {
			req.transaction = [];
			req.transaction.push(await base.transaction());
		} else {
			req.transaction.push(await base.transaction());
		}
	},
	...
	handleError: function (err, req, res, transaction) {
	...
	execError: function (req, res, httpCode, msg) {
		try {
			...
			RouterService.jsonErr(err, res, httpCode);
		}
	},
